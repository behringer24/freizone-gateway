...
- `POST /v1/push/send` -- signed (see Security model above). Body: `{"platform": "fcm" | "apns", "token": "..."}`. `200 {"status":"sent"}` · `400` bad platform/body · `401` bad/missing/revoked signature or replayed nonce · `410 {"code": "token_invalid"}` the upstream service reported this token **permanently** dead (app uninstalled, data cleared, wrong sender id, or a token the service rejects outright as malformed) · `501` platform recognized but not configured on this instance · `502` the upstream FCM/APNs call itself failed (currently always, for `apns`, since sending isn't implemented).
